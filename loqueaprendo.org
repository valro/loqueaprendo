...
*** Creando y fusionando ramas
    Forma rápida de crear una rama: git checkout -b get-dir
    Para crear esa rama en el repositorio remoto y sincronizr los dos
    repositorios: git push --set-upstream origin get-dir
    get-dir es el nombre de la rama que hemos creado.
    Las ramas de trabajo se pueden listar con git branch. Si queremos ver todas: git branch --all
    git branch -vv. Doble verbosidad
    checkout te permite pasar fuicheros de una rama a otra sin tener que fusionar la rama completa: por ejemplo queremos pasar un fichero a la rama gh-pages desde la rama master: 
    #+BEGIN_SRC sh
...
* LaTeX
** [[https://ondahostil.wordpress.com/2020/02/24/lo-que-he-aprendido-pdfjam-y-la-aneca/][pdfjam]]
** [[https://ondahostil.wordpress.com/2020/02/04/lo-que-he-aprendido-la-clase-standalone/][standalone class]]
* Linux. How do I know if a particular library is installed in Linux or not? 
** ldconfig
...
* Linux drivers
** Instalar drivers de windows con ndiswrapper
  [[https://www.taringa.net/+linux/instalar-drivers-de-windows-en-ubuntu_12u4yo][Instalar drivers de windows en ubuntu]]
* Linux From Scratch
...
* [[https://www.youtube.com/watch?v=Iya2b7F0WYk][¿Cómo instalar Debian paso a paso? | GNU/Linux Debian]]
*** Listado de cosas a hacer
    1. Elegir Advance modo gráfico
2. elegir lenguaje. situación. utf-8
3. configurar teclado.
4. detectar y montar cd-rom
5. cargar componentes del instalador. continuar
6. detectar hardware de red
7. configurar red automática
8.nombre del máquina: debian-vgs
9.nombre dominio: blanco
10.contraseñas si, si
11. contraseña de root
12. usuario normal: valgalsan
13.contraseña usuario
14.configurar el reloj:si
15.zona horario
16.particionado de discos: Manual
17. SCSI-Seleccion del disco
18.crear tabla de particiones: gpt
19.crear partición nueva
20.asignar 500MB al principio. sistema ext2.punto de montaje /boot
21.crear partición: root. 100 gb. ext4. punto de montaje: /
22.crear swap. 8 GB. principio.
23.crear home. ext4 punto de montaje /home.
24. Finalizar particionado. Escribir los cambios al disco.
25.Instalar el sistema base. Opcion por defecto.
26.drivers. instalar genéricos.
27.configurar el gestor de paquetes. NO. Un único disco.
28.Réplica Red. SI
29.protocolo por defecto.
30.seleccionar servidor de usa. alta disponibilidad.
31.Software no libre SI
32.Analizar repositorios de fuentes. NO-SI
33.actualizaciones de seguridad. actualizaciones de distribución
34.encuesta. NO
35.escritorio. GNOME
36.instalar cargdaor de arranque. SI (GRUB). Elegir el disco.remivibles.NO
37.Terminar la instalación
38.Horario UTC
39. Instalacion completa. Extraer el USB. Clic en continuar

40. install ndiswrapper

Personalizar debian
1. incluir usuario en sudoers
nano /etc/sudoers
añadir valgalsan
2. ampliar repositorios. software y updates.
3. personalizar debian
        retoques. gnome tweak tools
        animaciones...
        temas apt-install ark-shell. iconos
        aplicaciones al inicio
        barra superior
        extensiones. GS Connect. Dash to Dock
        teclado y raton
        tipografía
        tasksel
        openbox
...
* bspwm
** [[https://www.instructables.com/id/Bspwm-Installation-and-Configuration/][Bspwm Installation and Configuration]]
...
* Videos
** [[https://www.youtube.com/channel/UC0uTPqBCFIpZxlz_Lv1tk_g/search?query=ido][Protesilaos Stavrou]]
* Emacs  
** [[http://tuhdo.github.io/emacs-tutor.html#orgheadline15][Emacs Mini Manual (PART 1) - THE BASICS]]
** [[https://emacs.stackexchange.com/questions/5647/how-to-input-special-character-such-as-celsius-symbol-and-common-greek-letters][How to input special character, such as Celsius symbol, and common greek letters?]] 
   #+BEGIN_SRC elisp
   ...
   #+END_SRC
   por ejemplo, introducir las letras pi y epsilon
   #+BEGIN_SRC elisp
   M-x set-input-method greek
   Escribiendo: e, pi se obtiene: ε, π
   #+END_SRC
...
*** 
* Emacs. protesilaos. configuration
** init.el
  ...
  (org-babel-load-file (expand-file-name "~/.emacs.d/emacs-init.org"))
   ...
   #+END_SRC
* Emacs. Install from Source
  - Step one. Install dependencies
    #+BEGIN_SRC sh
    apt-get build-dep emacs24
    #+END_SRC
  - Step two. Check if ftp package is installed.
    #+BEGIN_SRC sh
    ...
    #+END_SRC
  - Step three. Make the current user own everything inside the folder (and the folder itself):
    #+BEGIN_SRC sh
    #sudo chown -R $USER ./Documents/linux-sources/emacs
    sudo chown -R $USER /path/to/directory
    #+END_SRC
  - Step four. Download the files.
    #+BEGIN_SRC sh
    ftp ftp.gnu.org anonymous
    #+END_SRC
    #+BEGIN_SRC 
    cd gnu/emacs
    get emacs-26.3.tar.xz
    get emacs-26.3.tar.xz.sig
    #+END_SRC
  - Step five. Uncompress the file.
    #+BEGIN_SRC sh
    tar -xvf emacs-26.3.tar.xz
    #+END_SRC
  - Step six. change to directory
    #+BEGIN_SRC sh
    cd emacs-26.3
    #+END_SRC
  ...
  - Step eight. make
    ...
    make
    #+END_SRC
  - Step nine. mahe install using /usr/local/valgalsan
    #+BEGIN_SRC 
    sudo make install prefix=/usr/local/valgalsan/emacs
    #+END_SRC
** Building emacs from git
*** [[http://ergoemacs.org/emacs/building_emacs_from_git_repository.html][building emcas from git]]
...
* Emacs packages
** [[https://github.com/emacs-helm/helm/wiki][Helm]]
* Windows 10
